...
class IndustryRegistry:
    ...
    def _normalize_addons(self, value) -> dict:
        ...
        if isinstance(value, list):
            ...
            for item in value:
                if isinstance(item, dict):
                    ...
                    if key:
                        out[key] = float(price or 0)
                elif isinstance(item, str):
                    out[item] = 0.0
            return out
        return {}

    def _normalize_quantity_fields(self, value) -> dict:
        if isinstance(value, dict):
            return value
        if isinstance(value, list):
            out: dict[str, dict] = {}
    ...
    def _validate_and_normalize(self, data: dict, file_path: Path) -> dict:
        ...
        data["quantity_fields"] = self._normalize_quantity_fields(data.get("quantity_fields", {}))
        data.setdefault("multipliers", {})
        data["addons"] = self._normalize_addons(data.get("addons", {}))
