...
def fetch(url,accept='text/html,*/*'):
    req=Request(url,headers={'User-Agent':UA,'Accept':accept,'Accept-Language':'en-AU,en;q=0.9'})
    with urlopen(req,timeout=25) as r:return r.status,r.read().decode('utf-8','replace')

def clean(s):
    s=htmllib.unescape(s or ''); s=re.sub(r'<[^>]+>',' ',s); return re.sub(r'\s+',' ',s).strip()
...
def event_from_text(text,url,published=None):
    text=clean(text)
    if not re.search(r'(tagged\s+(?:White|Bull|Tiger)\s+Shark|By Drone|By Lifeguard|SMART drumline)',text,re.I):return None
    when=parse_time(text) or published
    if not when:return None
    station,lat,lon=location(text); sm=re.search(r'(White|Bull|Tiger)\s+Shark',text,re.I); species=(sm.group(1).title()+' Shark') if sm else 'Unknown Shark'; tm=re.search(r'Shark\s+#?(\d+)',text,re.I); tag=tm.group(1) if tm else None
    cat='acoustic'
    ...
    return {'id':hashlib.sha1((text+url).encode()).hexdigest()[:20],'event_time':when,'category':cat,'species':species,'tag_id':tag,'station':station,'lat':lat,'lon':lon,'text':text,'source':'NSW SharkSmart public relay','source_url':url}

def parse_rss(xml,source):
    found=[]; root=ET.fromstring(xml)
    for item in root.findall('.//item'):
        title=clean(item.findtext('title')); desc=clean(item.findtext('description')); link=clean(item.findtext('link')); pub=None
        p=clean(item.findtext('pubDate'))
        if p:
            try:
                from email.utils import parsedate_to_datetime
                pub=parsedate_to_datetime(p).astimezone(timezone.utc).isoformat().replace('+00:00','Z')
            except:pass
        ev=event_from_text(title+' '+desc,link or source,pub)
        if ev:found.append(ev)
    return found

def nitter_rss():
    found=[]; errors=[]
    bases=['https://xcancel.com','https://nitter.net','https://nitter.poast.org','https://nitter.privacyredirect.com','https://nitter.tiekoetter.com']
    for base in bases:
        url=base+'/NSWSharkSmart/rss'
        try:
            status,xml=fetch(url,'application/rss+xml,text/xml,*/*')
            rows=parse_rss(xml,url); found+=rows
            if rows:break
            errors.append(url+': HTTP '+str(status)+', 0 parsed')
        except Exception as e:errors.append(url+': '+type(e).__name__+': '+str(e))
    return found,errors

def bing_rss():
    found=[]; errors=[]
    for q in ['site:x.com/NSWSharkSmart/status "DPI Fisheries advise" tagged Shark','"SharkSmart @NSWSharkSmart" "DPI Fisheries advise"','site:x.com/NSWSharkSmart/status "By Drone" shark']:
        url='https://www.bing.com/search?format=rss&count=50&q='+quote_plus(q)
        try:
            status,xml=fetch(url,'application/rss+xml,text/xml,*/*'); found+=parse_rss(xml,url)
        except Exception as e:errors.append('bing: '+type(e).__name__+': '+str(e))
    return found,errors

def dedupe(events):
    d={}
    for e in events:d[(e.get('tag_id'),e.get('station'),e.get('event_time'),e.get('category'))]=e
...
def main():
    ...
    events=[]; errors=[]
    for fn in (nitter_rss,bing_rss):
        a,er=fn();events+=a;errors+=er
        if len(events)>=5:break
    events=dedupe(events)
    if not events and old.get('events'):events=old['events'];errors.append('using last-good relay cache')
    out={'checked_at':datetime.now(timezone.utc).isoformat().replace('+00:00','Z'),'ok':bool(events),'events':events[:150],'event_count':len(events),'latest_event_time':events[0]['event_time'] if events else None,'errors':errors[-15:],'relay_version':'1.2'}
